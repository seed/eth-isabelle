...
"\<exists>r. bbtriple net
  ...
  apply (((sep_cancel, (clarsimp simp: word_rcat_simps memory_def[ where ind="0xA0"])?)+)|simp add: word_rcat_simps|rule conjI)
              apply (((sep_cancel, clarsimp?)+)|simp add:|rule conjI)
    apply (sep_imp_solve2)
    apply (sep_imp_solve2)
    apply (sep_imp_solve2)
