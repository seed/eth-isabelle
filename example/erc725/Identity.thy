...
Run Isabelle:
isabelle jedit -l Libs -d example/ -d Word_Lib/ -d lem/ example/erc725/Identity.thy
...
lemma len_input_minus_4_xs_data_execute_hash[simplified execute_hash_def]:
 "length xs \<le> 0x100000000 \<Longrightarrow>
  length (input_data execute_hash op_type to v xs) - unat ((4::w256) + of_nat (length xs)) = 128"
  apply (simp add: input_data_def bytestr_def execute_hash_def)
...
(* HERE *)
  ...
  using xs
          apply (subst (asm) unat_plus_if';(subst (asm) le_unat_uoi[where z="of_nat (2^64)"]; simp)+)
  ...
  apply (clarsimp?, order_sep_conj)
    ...
    apply (sep_imp_solve2)
    apply (sep_imp_solve2)
  apply (simp add: word_rcat_simps len_input_minus_4_xs_data_execute_hash[OF xs])
  apply (clarsimp?, order_sep_conj)
              apply (((sep_cancel, clarsimp?)+)|simp add:|rule conjI)
              apply (((sep_cancel, clarsimp?)+)|simp add:|rule conjI)
