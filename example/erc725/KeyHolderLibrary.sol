...
library KeyHolderLibrary {
    ...
    function init(KeyHolderData storage _keyHolderData)
        public
    {
        bytes32 _key = keccak256(abi.encodePacked(msg.sender)); // abi.encodePacked is like casting to bytes
        ...
        returns (bool success)
    {
        ...
        if (_approve == true) {
            ...
            success = _keyHolderData.executions[_id].to.call(abi.encodePacked(_keyHolderData.executions[_id].data, v));
            if (success) {
